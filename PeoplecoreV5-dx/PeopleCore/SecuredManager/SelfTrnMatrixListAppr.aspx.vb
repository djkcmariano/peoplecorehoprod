...
Partial Class Secured_TrnMatrixList
    ...
    Protected Sub PopulateGrid(Optional IsMain As Boolean = False)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ETrnCurriculum_WebManager", UserNo, Generic.ToInt(cboTabNo.SelectedValue), "", PayLocNo)
    ...
    Private Sub PopulateDetl()
        ...
        imgPhoto.ImageUrl = "~/SecuredManager/frmShowImage.ashx?tNo=" & Generic.ToInt(ViewState("TransNo")) & "&tIndex=2"

        Dim dt As DataTable
        dt = SQLHelper.ExecuteDataTable("ETrnMatrix_Web", UserNo, Generic.ToInt(ViewState("TransNo")), 0, 2, PayLocNo)
