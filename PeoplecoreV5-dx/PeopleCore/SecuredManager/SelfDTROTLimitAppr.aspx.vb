...
Partial Class SecuredManager_SelfDTROTLimit
    ...
    Protected Sub PopulateGrid()
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("EDTROTLimit_WebManager", UserNo, Generic.ToInt(cboTabNo.SelectedValue), PayLocNo)
    ...
    Protected Sub lnkSave_Click(sender As Object, e As EventArgs)
       Dim RetVal As Boolean = False
        ...
        dtx = SQLHelper.ExecuteDataTable("EDTROTLimit_WebValidate", UserNo, OTLimitNo, employeeno, Monthno, AppYear, MaxHrs, Limit, Me.txtReason.Text.ToString, statNo, IsCTO, PayLocNo, departmentNo)
        ...
        Dim fds As DataSet = SQLHelper.ExecuteDataSet("EDTROTLimit_WebSave_Manager", UserNo, OTLimitNo, employeeno, Monthno, AppYear, MaxHrs, Limit, Me.txtReason.Text.ToString, statNo, IsCTO, PayLocNo, departmentNo)
    ...
    Private Sub ApproveTransaction(tId As Integer, remarks As String, approvalStatNo As Integer)
        Dim fds As DataSet
        fds = SQLHelper.ExecuteDataSet("EDTROTLimit_WebApproved", UserNo, tId, approvalStatNo, remarks)
        If fds.Tables.Count > 0 Then
            If fds.Tables(0).Rows.Count > 0 Then
                Dim IsWithapprover As Boolean
                IsWithapprover = Generic.CheckDBNull(fds.Tables(0).Rows(0)("IsWithApprover"), clsBase.clsBaseLibrary.enumObjectType.IntType)
