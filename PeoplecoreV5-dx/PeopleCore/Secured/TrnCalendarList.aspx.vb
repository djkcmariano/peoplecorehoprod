...
Partial Class Secured_TrnCalendarList
    ...
    Private Sub PopulateSchedule(fmonth As Integer, fYear As Integer)
        ...
        ds = SQLHelper.ExecuteDataSet("ETrnTaken_Web_Calendar", UserNo, fmonth, fYear, Generic.CheckDBNull(ViewState("trntitleno"), clsBase.clsBaseLibrary.enumObjectType.IntType), statusno)
        grdCal.DataSource = ds
        grdCal.DataBind()

        lblMonth.Text = UCase(getMonth(fmonth)) & ", " & fYear.ToString

        cboMonthNo.Text = fmonth

        txtYear.Text = fYear

        cboTrnTitleNo.Text = IIf(ViewState("trntitleno") = "0", "", Generic.CheckDBNull(ViewState("trntitleno"), clsBase.clsBaseLibrary.enumObjectType.StrType))
    ...
    Private Sub PopulateDropDown()
        ...
        Try
            cboTrnTitleNo.DataSource = SQLHelper.ExecuteDataSet("xTable_Lookup", UserNo, "ETrnTitle", PayLocNo, "", "")
            cboTrnTitleNo.DataValueField = "tNo"
            cboTrnTitleNo.DataTextField = "tDesc"
            cboTrnTitleNo.DataBind()
        Catch ex As Exception
    ...
    Protected Sub lnkSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            ViewState("trntitleno") = Generic.CheckDBNull(cboTrnTitleNo.SelectedValue, clsBase.clsBaseLibrary.enumObjectType.IntType)
