...
Partial Class Secured_RptTemplateViewerDynamic_NestedFilter
    ...
    Dim AutoPostback_SP(10) As String
    Dim IsAutoPostBack As Integer
    Dim ReportFieldNext(10) As String

    Dim drpFilterno As New DropDownList
    ...
    End Sub
    Protected Sub filterNo_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            Dim tId As Integer
            Dim drpC As New DropDownList

            drpC = sender

            Dim drpfilterselect As New DropDownList
            'drpfilterselect.ID = ReportFieldNext(drpC.TabIndex)

            drpfilterselect = CType(pForm.FindControl(ReportFieldNext(drpC.TabIndex)), DropDownList)

            tId = Generic.ToInt(drpC.SelectedValue)
            Dim ds As New DataSet
            ds = SQLHelper.ExecuteDataSet("xTable_Lookup_Select", xPublicVar.xOnlineUseNo, AutoPostback_SP(drpC.TabIndex), tId)
            Try
                drpfilterselect.DataSource = ds
                drpfilterselect.DataTextField = "tDesc"
                drpfilterselect.DataValueField = "tNo"
                drpfilterselect.DataBind()
            Catch ex As Exception
            End Try
        ...
        If ReportParamTypeNo = 1 Then
            ...
            Else
                ...
                If TableViewName.Length > 0 Or Generic.CheckDBNull(TableViewName, Global.clsBase.clsBaseLibrary.enumObjectType.StrType) <> "" Then
                    ...
                    drp.DataBind()
                End If
                If IsAutoPostBack <> 0 Then
                    drp.AutoPostBack = True
                    drp.TabIndex = IsAutoPostBack
                    AddHandler drp.SelectedIndexChanged, AddressOf filterNo_SelectedIndexChanged
                End If
        ...
        If _ds.Tables.Count > 0 Then
            If _ds.Tables(0).Rows.Count > 0 Then
                ...
                For i As Integer = 0 To Rowcount - 1
                    ...
                    IsAutoPostBack = Generic.ToInt(_ds.Tables(0).Rows(i)("IsAutoPostback"))
                    AutoPostback_SP(IsAutoPostBack) = Generic.ToStr(_ds.Tables(0).Rows(i)("AutoPostback_SP"))
                    ReportFieldNext(IsAutoPostBack) = Generic.ToStr(_ds.Tables(0).Rows(i)("ReportFieldNext"))
