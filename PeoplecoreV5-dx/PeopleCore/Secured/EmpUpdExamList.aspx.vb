...
Partial Class Secured_EmpUpdExamList
    ...
    Private Sub PopulateGrid(Optional ByVal SortExp As String = "", Optional ByVal sordir As String = "")
        ...
        _dt = SQLHelper.ExecuteDataTable("EEmployeeUpdExam_Web", UserNo, Generic.ToInt(cboTabNo.SelectedValue), PayLocNo)
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("EEmployeeUpdExam_WebOne", UserNo, id)
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        ...
        Dim value As Integer = Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"EmployeeUpdExamNo"}))
        dt = SQLHelper.ExecuteDataTable("EEmployeeUpdExam_WebOne", UserNo, value)
        Generic.PopulateData(Me, "Panel1", dt)
        Highlight()
    ...
    Protected Sub lnkApprove_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"EmployeeUpdExamNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                SQLHelper.ExecuteDataSet("EEmployeeUpdExam_WebUpdate", UserNo, item, 2, PayLocNo)
                i = i + 1
            Next
    ...
    Protected Sub lnkDisapprove_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            ...
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                SQLHelper.ExecuteDataSet("EEmployeeUpdExam_WebUpdate", UserNo, item, 3, PayLocNo)
                i = i + 1
            Next
    ...
    Protected Sub lnkApprove2_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            ...
            If SQLHelper.ExecuteNonQuery("EEmployeeUpdExam_WebUpdate", UserNo, id, 2, PayLocNo) > 0 Then
    ...
    Protected Sub lnkDisapprove2_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            Dim id As Integer = Generic.ToInt(hifTransNo.Value)
            If SQLHelper.ExecuteNonQuery("EEmployeeUpdExam_WebUpdate", UserNo, id, 3, PayLocNo) > 0 Then
