...
Partial Class Secured_EHSProgramList
    ...
    Private Sub PopulateGrid(Optional IsMain As Boolean = False)
        ...
        _dt = SQLHelper.ExecuteDataTable("EEHSProgram_Web", UserNo, PayLocNo, tabOrder)
    ...
    Private Sub PopulateCombo()
        ...
        Try
            cboTabNo.DataSource = SQLHelper.ExecuteDataSet("EEHSStat_WebLookup", UserNo, PayLocNo)
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            Dim URL As String = Generic.GetFirstTab(Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"EHSProgramNo"})))
            If URL <> "" Then
    ...
    Protected Sub lnkSummary_Click(sender As Object, e As EventArgs)
        ...
        lnk = sender
        Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
        Response.Redirect("~/secured/CliEHSProgramEmpEdit.aspx?id=" & container.Grid.GetRowValues(container.VisibleIndex, New String() {"EHSProgramNo"}))
    End Sub

    Protected Sub lnkAdd_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd) Then
            Response.Redirect("~/secured/CliEHSProgramEdit.aspx?id=0")
        Else
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"EHSProgramNo"})
            Dim i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAuditCol("EEHSProgramDeti", UserNo, "EHSProgramNo", item)
                Generic.DeleteRecordAuditCol("EEHSProgramCost", UserNo, "EHSProgramNo", item)
                Generic.DeleteRecordAudit("EEHSProgram", UserNo, item)
                i = i + 1
            Next
            PopulateGrid(True)
