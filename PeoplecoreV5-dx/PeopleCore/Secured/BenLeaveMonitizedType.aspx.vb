...
Partial Class Secured_BenLeaveMonitizedType
    ...
    Protected Sub PopulateGrid()
        ...
        Try
            ...
            _dt = SQLHelper.ExecuteDataTable("ELeaveMonitizedType_Web", UserNo, Generic.ToInt(cboTabNo.SelectedValue), PayLocNo)
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ELeaveMonitizedType_WebOne", UserNo, id)
            For Each row As DataRow In dt.Rows
                Generic.PopulateData(Me, "Panel1", dt)
                Me.lnkSave.Enabled = Generic.ToInt(row("xIsEnabled"))
            Next
        Catch ex As Exception
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
            Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"LeaveMonitizedTypeNo", "IsEnabled", "xIsEnabled"})
            Dim iNo As Integer = Generic.ToInt(obj(0))
            Dim IsEnabled As Boolean = Generic.ToBol(obj(1))
            PopulateData(Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"LeaveMonitizedTypeNo"})))
            lnkSave.Enabled = Generic.ToBol(obj(2))
            Try
                cboPayLocNo.DataSource = SQLHelper.ExecuteDataSet("EPayLoc_WebLookup_Reference", UserNo, PayLocNo)
                cboPayLocNo.DataTextField = "tdesc"
                cboPayLocNo.DataValueField = "tNo"
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"LeaveMonitizedTypeNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("ELeaveMonitizedType", UserNo, item)
                i = i + 1
            Next
    ...
    Protected Sub lnkArchive_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            ...
            For Each item As Integer In fieldValues
                If SQLHelper.ExecuteNonQuery("ELeaveMonitizedType_WebArchive", UserNo, item, 1) > 0 Then
                    i = i + 1
                End If
            Next
    ...
    Private Function SaveRecord() As Boolean
        ...
        dt = SQLHelper.ExecuteDataTable("ELeaveMonitizedType_WebSave", UserNo, Generic.ToInt(txtCode.Text), txtLeaveMonitizedTypeCode.Text, txtLeaveMonitizedTypeDesc.Text, _
                                     Generic.ToDec(txtMinimum.Text), Generic.ToDec(txtAllowable.Text), Generic.ToDec(txtFactor.Text), 0, Generic.ToInt(Me.txtNoOfApplys.Text), Generic.ToInt(cboPayLocNo.SelectedValue))
        For Each row As DataRow In dt.Rows
