...
Partial Class Secured_EmpSchool
    ...
    Private Sub PopulateDropDown()
        ...
        End Try
        Try
            cboTabDetiNo.DataSource = SQLHelper.ExecuteDataSet("ETab_WebLookup", Generic.ToInt(Session("OnlineUserNo")), 14)
            cboTabDetiNo.DataTextField = "tDesc"
            cboTabDetiNo.DataValueField = "tno"
            cboTabDetiNo.DataBind()
        Catch ex As Exception
        End Try
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            dt = SQLHelper.ExecuteDataTable("ESchool_WebOne", UserNo, Generic.ToInt(iNo))
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        ...
        If SQLHelper.ExecuteNonQuery("ESchool_WebSave", UserNo, Generic.ToInt(txtCode.Text), Code, Description, Generic.ToInt(cboPayLocNo.SelectedValue), IsApplytoAll, IsArchived, txtEffectiveDate.Text, txtDocRef.Text, txtRemarks.Text) > 0 Then
            PopulateGrid()
...
#Region "Details"
    ...
    Private Sub PopulateGridDetl()
        Dim dt As DataTable
        dt = SQLHelper.ExecuteDataTable("ESchoolDeti_Web", UserNo, Generic.ToInt(ViewState("TransNo")), Generic.ToInt(cboTabDetiNo.SelectedValue))
    ...
    Protected Sub lnkDetail_Click(sender As Object, e As EventArgs)
        ...
        lnk = sender
        Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
        Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"tableNo", "Code"})
    ...
    Protected Sub lnkDeleteDetl_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdDetl.GetSelectedFieldValues(New String() {"SchoolDetiNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("ESchoolDeti", UserNo, Generic.ToInt(item))
                i = i + 1
            Next
    ...
    Protected Sub lnkEditDetl_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            lnk = sender
            Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
            Dim ino As Integer = Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"SchoolDetiNo"}))

            dt = SQLHelper.ExecuteDataTable("ESchoolDeti_WebOne", UserNo, ino)
    ...
    Protected Sub btnSaveDetl_Click(sender As Object, e As EventArgs)
        ...
        Dim RetVal As Integer = 0, xMessage As String = "", alertType As String = ""
        dt = SQLHelper.ExecuteDataTable("ESchoolDeti_WebValidate", UserNo, Generic.ToInt(txtCodeDeti.Text), txtSchoolDetiDesc.Text, Generic.ToInt(cboPayLocNo.SelectedValue), Generic.ToInt(ViewState("TransNo")))
        If dt.Rows.Count > 0 Then
            RetVal = Generic.ToInt(dt.Rows(0)("RetVal"))
            xMessage = Generic.ToStr(dt.Rows(0)("xMessage"))
            alertType = Generic.ToStr(dt.Rows(0)("alertType"))
        End If        
        ...
        If SQLHelper.ExecuteNonQuery("ESchoolDeti_WebSave", UserNo, Generic.ToInt(txtCodeDeti.Text), txtSchoolDetiDesc.Text, Generic.ToInt(ViewState("TransNo")), txtRemarksDeti.Text, Generic.ToInt(chkIsArchivedDeti.Checked)) > 0 Then
            PopulateGridDetl()
            MessageBox.Success(MessageTemplate.SuccessSave, Me)
        Else
            MessageBox.Critical(MessageTemplate.ErrorSave, Me)
