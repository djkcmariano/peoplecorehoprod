...
Partial Class Secured_PayAlphaList_Emp
    ...
    Private Sub PopulateGrid()
        ...
        _dt = SQLHelper.ExecuteDataTable("EAlphaEmployee_Web", UserNo, TransNo)
    ...
    Private Sub PopulateDataEntitled(id As Integer)
        Dim dt As DataTable
        dt = SQLHelper.ExecuteDataTable("EAlphaEmployee_WebOne", UserNo, id)
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete, "PayAlphaList.aspx", "EAlpha") Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"AlphaEmployeeNo"})
            Dim i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EAlphaEmployee", UserNo, item)
                i = i + 1
            Next
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit, "PayAlphaList.aspx", "EAlpha") Then
            ...
            PopulateDataEntitled(Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"AlphaEmployeeNo"})))
    ...
    Private Function SaveRecord() As Boolean
        Dim AlphaEmployeeno As Integer = Generic.ToInt(hifAlphaEmployeeNo.Value)
        Dim employeeNo As Integer = Generic.ToInt(Generic.Split(hifEmployeeNo.Value, 0))

        If SQLHelper.ExecuteNonQuery("EAlphaEmployee_WebSave", UserNo, AlphaEmployeeno, TransNo, employeeNo) > 0 Then
            SaveRecord = True
        Else
            SaveRecord = False
