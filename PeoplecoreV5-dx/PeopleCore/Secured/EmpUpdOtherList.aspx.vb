...
Partial Class Secured_EmpUpdOtherList
    ...
    Private Sub PopulateGrid(Optional ByVal SortExp As String = "", Optional ByVal sordir As String = "")
        ...
        _dt = SQLHelper.ExecuteDataTable("EEmployeeUpdOther_Web", UserNo, Generic.ToInt(cboTabNo.SelectedValue), PayLocNo)
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        ...
        Dim value As Integer = Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"EmployeeOtherUpdNo"}))
        dt = SQLHelper.ExecuteDataTable("EEmployeeUpdOther_WebOne", UserNo, value)
        Generic.PopulateData(Me, "Panel1", dt)
        Highlight()
        If IsDate(lblChargedDetiDate_Old.Text) Then : lblChargedDetiDate_Old.Text = DateTime.Parse(lblChargedDetiDate_Old.Text).ToShortDateString : End If
        If IsDate(lblChargedDetiDate.Text) Then : lblChargedDetiDate.Text = DateTime.Parse(lblChargedDetiDate.Text).ToShortDateString : End If
        If IsDate(lblSoloParentExpiryDate_Old.Text) Then : lblSoloParentExpiryDate_Old.Text = DateTime.Parse(lblSoloParentExpiryDate_Old.Text).ToShortDateString : End If
        If IsDate(lblSoloParentExpiryDate.Text) Then : lblSoloParentExpiryDate.Text = DateTime.Parse(lblSoloParentExpiryDate.Text).ToShortDateString : End If
    ...
    Protected Sub lnkApprove_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"EmployeeOtherUpdNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                SQLHelper.ExecuteDataSet("EEmployeeUpdOther_WebUpdate", UserNo, item, 2, PayLocNo)
                i = i + 1
            Next
    ...
    Protected Sub lnkDisapprove_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            ...
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                SQLHelper.ExecuteDataSet("EEmployeeUpdOther_WebUpdate", UserNo, item, 3, PayLocNo)
                i = i + 1
            Next
    ...
    Protected Sub lnkApprove2_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            ...
            If SQLHelper.ExecuteNonQuery("EEmployeeUpdOther_WebUpdate", UserNo, id, 2, PayLocNo) > 0 Then
    ...
    Protected Sub lnkDisapprove2_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            Dim id As Integer = Generic.ToInt(hifTransNo.Value)
            If SQLHelper.ExecuteNonQuery("EEmployeeUpdOther_WebUpdate", UserNo, id, 3, PayLocNo) > 0 Then
