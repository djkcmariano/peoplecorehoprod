...
Partial Class Secured_TrnSpeakerList
    ...
    '        dt = SQLHelper.ExecuteDataTable("ETrnSpeaker_Web", UserNo, Filter1.SearchText, PayLocNo)
    ...
    Protected Sub PopulateGrid()
        ...
        Try
            ...
            _dt = SQLHelper.ExecuteDataTable("ETrnSpeaker_Web", UserNo, PayLocNo)
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ETrnSpeaker_WebOne", UserNo, id)
    ...
    '        Dim dt As DataTable
    '        dt = SQLHelper.ExecuteDataTable("ETrnSpeaker_WebOne", UserNo, Generic.ToInt(ib.CommandArgument))
    '        For Each row As DataRow In dt.Rows
    '            Generic.PopulateData(Me, "pnlPopupMain", dt)
    '        Next
    '        mdlMain.Show()
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        ...
        Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"TrnSpeakerNo", "IsEnabled"})
        Dim iNo As Integer = Generic.ToInt(obj(0))
        Dim IsEnabled As Boolean = Generic.ToBol(obj(1))
        PopulateData(container.Grid.GetRowValues(container.VisibleIndex, New String() {"TrnSpeakerNo"}))
    ...
    '                Generic.DeleteRecordAudit("ETrnSpeaker", UserNo, Generic.ToInt(ib.CommandArgument))
    '                Count = Count + 1
    '            End If
    '        Next
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        Dim chk As New CheckBox, ib As New ImageButton, Count As Integer = 0
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"TrnSpeakerNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("ETrnSpeaker", UserNo, item)
                i = i + 1
            Next
    ...
    Private Function SaveRecord() As Boolean
        ...
        Dim TrnSpeakerNo As Integer = Generic.ToInt(txtCode.Text)
        Dim TrnSpeakerDesc As String = Generic.CheckDBNull(Me.txtTrnSpeakerDesc.Text, clsBase.clsBaseLibrary.enumObjectType.StrType)
        Dim Title As String = Generic.CheckDBNull(Me.txtTitle.Text, clsBase.clsBaseLibrary.enumObjectType.StrType)
        ...
        If SQLHelper.ExecuteNonQuery("ETrnSpeaker_WebSave", UserNo, TrnSpeakerNo, TrnSpeakerDesc, Title, Address, PhoneNo, FaxNo, Email, Remarks, Generic.ToInt(cboPayLocNo.SelectedValue)) > 0 Then
            SaveRecord = True
        Else
            SaveRecord = False
        End If
