...
Partial Class Secured_CarCompInterventionType
    ...
    '        dt = SQLHelper.ExecuteDataTable("ECompInterventionType_Web", UserNo, Filter1.SearchText, PayLocNo)
    ...
    '        ib = sender
    '        Dim dt As DataTable
    '        dt = SQLHelper.ExecuteDataTable("ECompInterventionType_WebOne", UserNo, Generic.ToInt(ib.CommandArgument))
    ...
    '                Generic.DeleteRecordAudit("ECompInterventionType", UserNo, Generic.ToInt(ib.CommandArgument))
    '                Count = Count + 1
    '            End If
    '        Next
    ...
    '    Dim CompInterventionTypeNo As Integer = Generic.ToInt(Me.txtCompInterventionTypeNo.Text)
    '    Dim CompInterventionTypeCode As String = Generic.ToStr(Me.txtCompInterventionTypeCode.Text)
    '    Dim CompInterventionTypeDesc As String = Generic.ToStr(Me.txtCompInterventionTypeDesc.Text)
    '    Dim Isfortraining As Boolean = Generic.ToBol(Me.txtIsfortraining.Checked)

    '    If SQLHelper.ExecuteNonQuery("ECompInterventionType_WebSave", UserNo, CompInterventionTypeNo, CompInterventionTypeCode, CompInterventionTypeDesc, Isfortraining, PayLocNo) > 0 Then
    '        Retval = True
    '    Else
    '        Retval = False
    '    End If
    ...
    Private Sub PopulateGrid()
        ...
        _dt = SQLHelper.ExecuteDataTable("ECompInterventionType_Web", UserNo, PayLocNo)
...
#Region "********Main*******"
    Protected Sub lnkEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
                ...
                'i = Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"CompInterventionTypeNo"}))
                Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
                Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"CompInterventionTypeNo", "IsEnabled"})
                Dim iNo As Integer = Generic.ToInt(obj(0))
                Dim IsEnabled As Boolean = Generic.ToBol(obj(1))

                Dim dt As DataTable
                dt = SQLHelper.ExecuteDataTable("ECompInterventionType_WebOne", UserNo, Generic.ToInt(iNo))
                For Each row As DataRow In dt.Rows
                    Generic.PopulateData(Me, "pnlPopupMain", dt)
                Next
    ...
    Protected Sub lnkDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkDelete.Click
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"CompInterventionTypeNo"})
            Dim str As String = ""
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("ECompInterventionType", UserNo, item)
    ...
    Private Function SaveRecord() As Integer
        ...
        If SQLHelper.ExecuteNonQuery("ECompInterventionType_WebSave", UserNo, CompInterventionTypeNo, CompInterventionTypeCode, CompInterventionTypeDesc, Isfortraining, Generic.ToInt(cboPayLocNo.SelectedValue)) > 0 Then
            Retval = True
        Else
            Retval = False
        End If
