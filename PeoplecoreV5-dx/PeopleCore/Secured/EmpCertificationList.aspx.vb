...
Partial Class Secured_EmpCertificationList
    ...
    Private Sub PopulateGrid(Optional IsMain As Boolean = False, Optional ByVal SortExp As String = "", Optional ByVal sordir As String = "")
        Try
            ...
            _ds = sqlHelper.ExecuteDataset("ECertification_Web", xPublicVar.xOnlineUseNo, Generic.CheckDBNull(Viewstate(xScript & "filter"), Global.clsBase.clsBaseLibrary.enumObjectType.StrType), Session("xPayLocNo"))
            _dt = _ds.Tables(0)
    ...
    Private Sub populatedata()
        Dim _ds As New DataSet
        _ds = sqlHelper.ExecuteDataset("ECertification_WebOne", xPublicVar.xOnlineUseNo, Generic.CheckDBNull(Viewstate(xScript & "No"), clsBase.clsBaseLibrary.enumObjectType.IntType))

        If Generic.CheckDBNull(Viewstate(xScript & "No"), clsBase.clsBaseLibrary.enumObjectType.IntType) > 0 Then
            Me.txtReferenceNo.Text = Pad.PadZero(8, ViewState(xScript & "No"))
        Else
            Me.txtReferenceNo.Text = "Autonumber"
        End If
        ...
        If _ds.Tables.Count > 0 Then
            If _ds.Tables(0).Rows.Count > 0 Then
                ...
                'Me.txtCertificationDesc.Text = Generic.CheckDBNull(_ds.Tables(0).Rows(0)("CertificationDesc"), clsBase.clsBaseLibrary.enumObjectType.StrType)
                'Me.cboCertificationTypeNo.Text = Generic.CheckDBNull(_ds.Tables(0).Rows(0)("CertificationTypeNo"), clsBase.clsBaseLibrary.enumObjectType.IntType)
                'hifEmployeeNo.Value = Generic.CheckDBNull(_ds.Tables(0).Rows(0)("SignatoryNo"), clsBase.clsBaseLibrary.enumObjectType.IntType)
            End If
        End If
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        xPublicVar.xTablename = Generic.CheckDBNull(Session("xTablename"), Global.clsBase.clsBaseLibrary.enumObjectType.StrType)
    ...
    Private Function SaveRecord() As Integer
        ...
        If SQLHelper.ExecuteNonQuery("ECertification_WebSave",
                                   xPublicVar.xOnlineUseNo, Generic.CheckDBNull(ViewState(xScript & "No"), clsBase.clsBaseLibrary.enumObjectType.IntType),
                                   Me.txtCertificationDesc.Text.ToString,
                                   Generic.CheckDBNull(Me.cboCertificationTypeNo.SelectedValue, clsBase.clsBaseLibrary.enumObjectType.IntType),
                                   Generic.CheckDBNull(hifEmployeeNo.Value, clsBase.clsBaseLibrary.enumObjectType.IntType), Session("xPayLocNo")) > 0 Then
            SaveRecord = True
