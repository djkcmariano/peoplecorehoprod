...
Partial Class Secured_PayBankDiskEdit
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If cboBankTypeNo.SelectedValue = 9 Then
            lnkCreate.Text = "Online Disk"
            lnkCreateOl.Visible = True
            lnkCreateOl.Text = "Offline Disk"
        End If
    ...
    Protected Sub lnkCreate_Click(sender As Object, e As System.EventArgs)
        Try
            If TransNo > 0 Then
                ...
                ElseIf Me.cboBankTypeNo.SelectedValue = 5 Then 'BDO
                    GeneratepayrollDisk_BDO()
                ElseIf Me.cboBankTypeNo.SelectedValue = 2 Then 'MetroBank
                    GeneratepayrollDisk_MetroBank()
                ElseIf Me.cboBankTypeNo.SelectedValue = 7 Then 'DBP
                    GeneratepayrollDisk_DBP()
                    'ElseIf Me.cboBankTypeNo.SelectedValue = 9 Then 'World partners Bank
                    '    GeneratepayrollDisk_WorldPartnersBank()
                ElseIf Me.cboBankTypeNo.SelectedValue = 6 Then 'Union Bank
                    GeneratepayrollDisk_UnionBank()
                ElseIf Me.cboBankTypeNo.SelectedValue = 8 Then 'PNB
                    GeneratepayrollDisk_PNB()
                ElseIf Me.cboBankTypeNo.SelectedValue = 9 Then 'East West
                    GeneratepayrollDisk_EastWest()
                ElseIf Me.cboBankTypeNo.SelectedValue = 13 Then 'ONB
                    GeneratepayrollDisk_ONB()
                ElseIf Me.cboBankTypeNo.SelectedValue = 16 Then 'ChinaBank
                    GeneratepayrollDisk_ChinaBank()
                End If
            Else
    ...
    Protected Sub lnkCreateOL_Click(sender As Object, e As System.EventArgs)
        Try
            If TransNo > 0 Then
                If Me.cboBankTypeNo.SelectedValue = 9 Then 'East West
                    GeneratepayrollDisk_EastWestOL()
                End If
            Else
                MessageBox.Warning("Unable to create no voucher defined.", Me)
            End If
...
#Region "********BPI"
    ...
    Private Sub GeneratepayrollDisk_BPI()
        ...
        Try
            ...
            Dim dsComp As New DataSet
            dsComp = SQLHelper.ExecuteDataSet("select * from dbo.EPayBankDiskRef where banktypeno=" & Generic.CheckDBNull(Me.cboBankTypeNo.SelectedValue, clsBase.clsBaseLibrary.enumObjectType.IntType))
            If dsComp.Tables.Count > 0 Then
                If dsComp.Tables(0).Rows.Count > 0 Then
                    xPABranchCode = Generic.CheckDBNull(dsComp.Tables(0).Rows(0)("BranchCode_PayrollAccount"), clsBase.clsBaseLibrary.enumObjectType.StrType)
                    xAccountNo = Generic.CheckDBNull(dsComp.Tables(0).Rows(0)("AccountNumber"), clsBase.clsBaseLibrary.enumObjectType.StrType)
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        If faccountno.Length < 10 Then
                            faccountno = Pad.PadZero(10, faccountno)
                        End If
                        WriteFile.WriteLine("00" & faccountno & Space(6) & xAmount)
                    Next
                End If
            End If
...
#Region "********Metro Bank"
    ...
    Private Sub GeneratepayrollDisk_MetroBank()
        ...
        Try
            ...
            dsComp = Nothing
            xfilename = tpath & "\" & xCompanyCode & "_MetroBank.txt"
            FileHolder = New FileInfo(xfilename)
            WriteFile = FileHolder.CreateText()
            ...
            Dim ds As New DataSet
            ds = SQLHelper.ExecuteDataSet("select a.bankaccountno as bankaccountno,a.amount as Net,B.EmployeeCode as EmployeeCode from EpaybankDiskDeti A LEFT OUTER JOIN EEmployee B on A.EmployeeNo=B.EmployeeNo where PayBankDiskNo = " & Me.txtPayBankDiskNo.Text)
            If ds.Tables.Count > 0 Then
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        xFHeadCount = xFHeadCount + 1
                        tnet = Generic.CheckDBNull(ds.Tables(0).Rows(i)("Net"), clsBase.clsBaseLibrary.enumObjectType.IntType)
                        xEmployeeCode = Generic.CheckDBNull(ds.Tables(0).Rows(i)("EmployeeCode"), clsBase.clsBaseLibrary.enumObjectType.StrType)

                        faccountno = Generic.CheckDBNull(Replace(ds.Tables(0).Rows(i)("bankaccountno"), "-", ""), clsBase.clsBaseLibrary.enumObjectType.StrType)
                        If tnet.ToString.Length > 8 Then
                            xAmount = Replace(tnet.ToString, ".", "")
                        End If
                        If tnet.ToString.Length <= 8 Then
                            xAmount = Pad.PadNetPay(tnet, 7)
                        End If

                        'xAmount = tnet.ToString("N")
                        If faccountno.Length < 10 Then
                            faccountno = Pad.PadZero(10, faccountno)
                        End If
                        ...
                        WriteFile.WriteLine(xEmployeeCode & xBankCode & faccountno & xAmount)
                    Next
                End If
            End If
            ...
            OpenText("../secured/Disk/" & xCompanyCode & "_MetroBank.txt")
...
#Region "********PNB"
    ...
    Private Sub GeneratepayrollDisk_PNB()
        ...
        Try
            ...
            Dim ds As New DataSet
            ds = SQLHelper.ExecuteDataSet("EPayBankDiskDeti_WebCreateDisk_PNB", Me.txtPayBankDiskNo.Text, Me.cboPayClassNo.SelectedValue, Me.cboBankTypeNo.SelectedValue)
            If ds.Tables.Count > 0 Then
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        xFHeadCount = xFHeadCount + 1
...
#Region "********EAST WEST"
    ...
    Private Sub GeneratepayrollDisk_EastWest()
        ...
        Try
            ...
            dsComp = Nothing
            xfilename = tpath & "\" & xCompanyCode & "_EastWest.csv"
            FileHolder = New FileInfo(xfilename)
            WriteFile = FileHolder.CreateText()

            WriteFile.WriteLine("HDR,Account Number,Amount,Remarks")
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        If faccountno.Length < 10 Then
                            faccountno = Pad.PadZero(10, faccountno)
                        End If
                        WriteFile.WriteLine("DTL," & faccountno & "," & xAmount & "," & "CONFIDENTIAL")
                    Next
                End If
            End If
            ...
            'footer
            xFxTotal = Replace(xFxTotal, ",", "")
            WriteFile.WriteLine("TLR," & xFHeadCount & "," & xFxTotal)

            WriteFile.Close()
            OpenText("../secured/Disk/" & xCompanyCode & "_EastWest.csv")
...
#Region "********EAST WEST OL"
    ...
    Private Sub GeneratepayrollDisk_EastWestOL()
        ...
        Try
            ...
            dsComp = Nothing
            xfilename = tpath & "\" & xCompanyCode & "_EastWestOL.txt"
            FileHolder = New FileInfo(xfilename)
            WriteFile = FileHolder.CreateText()

            Dim ds As New DataSet
            ds = SQLHelper.ExecuteDataSet("select bankaccountno,amount as net, convert(Varchar,Paydate,112) paydate from dbo.EPayBankDiskDeti A INNER JOIN dbo.EPayBankDisk B on A.paybankdiskno = B.paybankdiskno where A.paybankdiskno=" & Me.txtPayBankDiskNo.Text)
            If ds.Tables.Count > 0 Then
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        xFHeadCount = xFHeadCount + 1
                        ...
                        faccountno = Generic.CheckDBNull(Replace(ds.Tables(0).Rows(i)("bankaccountno"), "-", ""), clsBase.clsBaseLibrary.enumObjectType.StrType)
                        xAmount = Replace(FormatNumber(tnet, 2).ToString, ",", "")
                        xAmount = Replace(xAmount, ".", "")
                        xAmount = Pad.PadZero(7, xAmount)
                        paydate = Generic.CheckDBNull(Replace(ds.Tables(0).Rows(i)("paydate"), "/", ""), clsBase.clsBaseLibrary.enumObjectType.StrType)
                        If faccountno.Length < 10 Then
                            faccountno = Pad.PadZero(10, faccountno)
                        End If
                        WriteFile.WriteLine(Pad.PadSpaceRightLeft(faccountno, 14, True) & Pad.PadSpaceRightLeft(xAmount, 10, True) & Pad.PadSpaceRightLeft(paydate, 8, False))
                    Next
                End If
            End If
            ...
            WriteFile.Close()
            OpenText("../secured/Disk/" & xCompanyCode & "_EastWestOL.txt")
...
#Region "********ONB"
    ...
    Private Sub GeneratepayrollDisk_ONB()
        ...
        Try
            ...
            dsComp = Nothing
            xfilename = tpath & "\" & xCompanyCode & "_ONB.csv"
            FileHolder = New FileInfo(xfilename)
            WriteFile = FileHolder.CreateText()

            WriteFile.WriteLine("Account Number,Amount")
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        If faccountno.Length < 10 Then
                            faccountno = Pad.PadZero(10, faccountno)
                        End If
                        WriteFile.WriteLine(faccountno & "," & xAmount)
                    Next
                End If
            End If
            ...
            WriteFile.Close()
            OpenText("../secured/Disk/" & xCompanyCode & "_ONB.csv")
...
#Region "********CHINA BANK"
    ...
    Private Sub GeneratepayrollDisk_Chinabank()
        Dim FileHolder As FileInfo
        Dim WriteFile As StreamWriter
        Dim tpath As String = Page.MapPath("Disk") '"c:\Payroll Diskette"
        Dim xfilename As String '= path & "\" & "SSSLoan-" & Format(Now, "MMMMdd") & ".TXT"
        ...
        Try
            ...
            xFxTotals = Pad.PadNetPay(Generic.CheckDBNull(CStr(Format((xftotalS), "Fixed")), clsBase.clsBaseLibrary.enumObjectType.StrType), 12)
            xFxTotalC = Pad.PadNetPay(Generic.CheckDBNull(CStr(Format((xftotalC), "Fixed")), clsBase.clsBaseLibrary.enumObjectType.StrType), 12)
            xFxTotalT = Pad.PadNetPayDec(Generic.CheckDBNull(CStr(Format((xftotalS), "Fixed")), clsBase.clsBaseLibrary.enumObjectType.StrType), 11, 2)
            xFxTotalT = Pad.PadZero(14, xFxTotalT)
            Dim tRecordCode As String
            Dim tTransactionCode As String
            Dim xAmount As String, tCurrencyCode As String, xFxTotal As String
            Dim faccountno As String = ""
            ...
            If ds.Tables.Count > 0 Then
                If ds.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        faccountno = Pad.PadZero(10, Generic.CheckDBNull(Replace(ds.Tables(0).Rows(i)("bankaccountno"), "-", ""), clsBase.clsBaseLibrary.enumObjectType.StrType))
                        xAmount = Pad.PadNetPayDec(Generic.CheckDBNull(CStr(Format(ds.Tables(0).Rows(i)("Net"), "Fixed")), clsBase.clsBaseLibrary.enumObjectType.StrType), 11, 2)
                        Accounthashtotal = Accounthashtotal + CType(faccountno, Double)
                        faccountno = Pad.PadZero(10, faccountno)
                        xAmount = Pad.PadZero(14, xAmount)
                        'If Mid(faccountno, 6, 1) = "5" Then
                        '    faccountno = Mid(faccountno, 1, 5) & "6" & Mid(faccountno, 7, 4)
                        'End If
                        dhash1 = Mid(faccountno, 5, 2)
                        ...
                        totalhash = totalhash + dhastotal
                        WriteFile.WriteLine("*" & faccountno & xAmount & "C" & fBranchCode & "*")
                    Next
                End If
            End If
            ...
            WriteFile.WriteLine("*" & xAccountNo & xFxTotalT & "D" & fBranchCode & "*")
