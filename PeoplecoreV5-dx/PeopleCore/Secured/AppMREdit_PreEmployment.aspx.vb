...
Partial Class Secured_AppMREdit_PreEmployment
...
#Region "Main"
    Protected Sub PopulateGrid()
        Try
            ...
            If Generic.ToInt(ViewState("TransNo")) = 0 And dt.Rows.Count > 0 Then
                ...
                ViewState("IsEnabled") = grdMain.DataKeys(0).Values(2).ToString()
            End If
    ...
    Protected Sub lnkView_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            ViewState("TransNo") = grdMain.DataKeys(gvrow.RowIndex).Values(0).ToString()
            ViewState("TransCode") = grdMain.DataKeys(gvrow.RowIndex).Values(1).ToString()
            ViewState("IsEnabled") = grdMain.DataKeys(gvrow.RowIndex).Values(2).ToString()
            PopulateDetl()
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim tcount As Integer, SaveCount As Integer = 0, i As Integer = 0
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd) Then
            ...
            For tcount = 0 To Me.grdMain.Rows.Count - 1
                ...
                Dim chk = CType(grdMain.Rows(tcount).FindControl("txtxIsSelectMain"), CheckBox)


                If chk.Checked = True Then
                    If Not cboRequiredStatNo Is Nothing Then
                        If StatusNo = 1 Then
...
#Region "Detail"
    Protected Sub PopulateDetl(Optional StatusNo As Integer = 0)
        Try
            ...
            btnUpdateDetl.Enabled = Generic.ToBol(ViewState("IsEnabled"))
            btnAddDetl.Enabled = Generic.ToBol(ViewState("IsEnabled"))
            btnDeleteDetl.Enabled = Generic.ToBol(ViewState("IsEnabled"))
        Catch ex As Exception
    ...
    Protected Sub btnUpdateDetl_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim lbl As New Label, chk As New CheckBox, chk2 As New CheckBox
        Dim tcount As Integer, SaveCount As Integer = 0
        Dim xds As New DataSet, txt As New TextBox, txtR As New TextBox
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd) Then
            ...
            For tcount = 0 To Me.grdDetl.Rows.Count - 1
                ...
                txt = CType(grdDetl.Rows(tcount).FindControl("txtDateSubmitted"), TextBox)
                chk2 = CType(grdDetl.Rows(tcount).FindControl("txtIschk"), CheckBox)
                txtR = CType(grdDetl.Rows(tcount).FindControl("txtRemarks"), TextBox)

                Dim tno As Integer = Generic.ToInt(lbl.Text)
                Dim IsSubmitted As Boolean = Generic.ToBol(chk.Checked)
                Dim DateSumitted As String = Generic.ToStr(txt.Text)
                Dim Remarks As String = Generic.ToStr(txtR.Text)

                If Not chk Is Nothing Then

                    If SQLHelper.ExecuteNonQuery("EApplicantChecklist_WebUpdate", UserNo, tno, IsSubmitted, DateSumitted, Remarks) > 0 Then
                        SaveCount = SaveCount + 1
                    End If
    ...
    Protected Sub btnBatch_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            For i As Integer = 0 To Me.grdMain.Rows.Count - 1
                chk = CType(grdMain.Rows(i).FindControl("txtxIsSelectMain"), CheckBox)
                If chk.Checked = True Then
                    Count = Count + 1
                End If
            Next
