...
Partial Class Secured_PayMainDeductMassList
    ...
    Private Sub PopulateGrid()
        ...
        _dt = SQLHelper.ExecuteDataTable("EPayMainDeductMass_Web", UserNo, tstatus, PayLocNo)
    ...
    Private Sub PopulateGridDetl(id As Integer)
        ...
        Dim dt As DataTable
        dt = SQLHelper.ExecuteDataTable("EPayMainDeductMassDeti_Web", UserNo, id, tstatus)
...
#Region "********Main*******"
    ...
    Protected Sub lnkDetail_Click(sender As Object, e As EventArgs)
        ...
        Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"PayMainDeductMassNo", "Code"})
    ...
    Protected Sub lnkDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"PayMainDeductMassNo"})
            Dim str As String = ""
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAuditCol("EPayMainDeductMassDeti", UserNo, "PayMainDeductMassNo", CType(item, Integer))
                Generic.DeleteRecordAudit("EPayMainDeductMass", UserNo, CType(item, Integer))
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        Try
            If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
                ...
                lnk = sender
                Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
                Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"PayMainDeductMassNo", "IsEnabled"})
                i = Generic.ToInt(obj(0))
                IsEnabled = Generic.ToBol(obj(1))

                Dim dt As DataTable
                dt = SQLHelper.ExecuteDataTable("EPayMainDeductMass_WebOne", UserNo, Generic.ToInt(i))
    ...
    Protected Sub lnkSave_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd) Then
            ...
            Dim tno As Integer = Generic.ToInt(Me.txtPayMainDeductMassNo.Text)
            ...
            If SQLHelper.ExecuteNonQuery("EPayMainDeductMass_WebSave", UserNo, tno, PayScheduleNo, PayTypeNo, PayDeductTypeNo, Description, Amount, IsApplytoall, PayClassNo, PayLocNo) > 0 Then
                Retval = True
            Else
                Retval = False
            End If
    ...
    Protected Sub lnkSuspend_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            Dim str As String = ""
            For Each item As Integer In fieldValues
                SQLHelper.ExecuteNonQuery("EPayMainDeductMass_WebSuspend", UserNo, CType(item, Integer), action)
                DeleteCount = DeleteCount + 1
            Next
    ...
    Protected Sub lnkSuspend_Indi_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
            ...
            Dim fieldValues As List(Of Object) = grdDetl.GetSelectedFieldValues(New String() {"PayMainDeductMassDetiNo"})
            Dim str As String = ""
            For Each item As Integer In fieldValues
                SQLHelper.ExecuteNonQuery("EPayMainDeductMassDeti_WebSuspend", UserNo, CType(item, Integer), action)
                DeleteCount = DeleteCount + 1
            Next
...
#Region "********Detail********"
    ...
    Protected Sub lnkDeleteDetl_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            ...
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EPayMainDeductMassDeti", UserNo, CType(item, Integer))
                i = i + 1
            Next
    ...
    Protected Sub lnkEditDetl_Click(sender As Object, e As EventArgs)
        Try
            If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
                ...
                lnk = sender
                Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
                i = Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"PayMainDeductMassDetiNo"}))

                Dim dt As DataTable
                dt = SQLHelper.ExecuteDataTable("EPayMainDeductMassDeti_WebOne", UserNo, Generic.ToInt(i))
                For Each row As DataRow In dt.Rows
    ...
    Protected Sub lnkSaveDetl_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd) Then
            ...
            Dim PayMainDeductMassDetiNo As Integer = Generic.ToInt(Me.txtPayMainDeductMassDetiNo.Text)
            ...
            dt = SQLHelper.ExecuteDataTable("EPayMainDeductMassDeti_WebValidate", UserNo, PayMainDeductMassDetiNo, tno, EmployeeNo, Amount, StartDate, EndDate)
            For Each row As DataRow In dt.Rows
            ...
            If SQLHelper.ExecuteNonQuery("EPayMainDeductMassDeti_WebSave", UserNo, PayMainDeductMassDetiNo, tno, EmployeeNo, Amount, StartDate, EndDate) > 0 Then
                Retval = True
            Else
                Retval = False
            End If
...
#Region "********Append*******"
    ...
    Protected Sub lnkSaveAppend_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd) Then
            ...
            Dim dt As DataTable
            Dim invalid As Boolean = True, messagedialog As String = "", alerttype As String = ""
            dt = SQLHelper.ExecuteDataTable("EPayMainDeductMassDeti_WebAppend_Validate", UserNo, tno, FilteredbyNo, FilterValue, Amount, StartDate, EndDate)
            For Each row As DataRow In dt.Rows
                invalid = Generic.ToBol(row("Invalid"))
                messagedialog = Generic.ToStr(row("MessageDialog"))
                alerttype = Generic.ToStr(row("AlertType"))
            ...
            If SQLHelper.ExecuteNonQuery("EPayMainDeductMassDeti_WebAppend", UserNo, tno, FilteredbyNo, FilterValue, Amount, StartDate, EndDate, PayLocNo) > 0 Then
                Retval = True
            Else
                Retval = False
            End If
