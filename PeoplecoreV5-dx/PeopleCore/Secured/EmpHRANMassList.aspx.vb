...
Partial Class Secured_EmpHRANMassList
    ...
    Protected Sub PopulateGrid()               
        ...
        End If
        _dt = SQLHelper.ExecuteDataTable("EHRANMass_Web", UserNo, PayLocNo, Generic.ToInt(cboTabNo.SelectedValue))
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"HRANMassNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EHRANMass", UserNo, item)
                i = i + 1
            Next
    ...
    Protected Sub lnkPost_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"HRANMassNo", "IsReady"})
            Dim str As String = ""
            For Each item As Object In fieldValues
                If Generic.ToBol(item(1)) Then
                    If SQLHelper.ExecuteNonQuery("EHRANMass_WebPost", UserNo, Generic.ToInt(item(0)), PayLocNo) > 0 Then
                        count = count + 1
                    End If
                Else
            ...
            Else
                MessageBox.Warning("Unable to post transaction.", Me)
            End If
