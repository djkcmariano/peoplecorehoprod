...
Partial Class Secured_DTRLeaveApplicationMassList
...
#Region "Main"
    Protected Sub PopulateGrid(Optional IsMain As Boolean = False)
        Try
            ...
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ELeaveApplicationMass_Web", UserNo, Generic.ToInt(cboTabNo.SelectedValue), PayLocNo)
            grdMain.DataSource = dt
            grdMain.DataBind()

            If ViewState("TransNo") = 0 Or IsMain = True Then
                Dim obj As Object() = grdMain.GetRowValues(grdMain.VisibleStartIndex(), New String() {"LeaveApplicationMassNo", "LeaveApplicationMassTransNo"})
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ELeaveApplicationMass_WebOne", UserNo, id)
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        AccessRights.CheckUser(UserNo, "DTRLeaveApplication.aspx", "ELeaveApplication")
    ...
    Protected Sub lnkAdd_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd, "DTRLeaveApplication.aspx", "ELeaveApplication") Then
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit, "DTRLeaveApplication.aspx", "ELeaveApplication") Then
            Dim lnk As New LinkButton
            lnk = sender
            Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
            PopulateData(Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"LeaveApplicationMassNo"})))
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete, "DTRLeaveApplication.aspx", "ELeaveApplication") Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"LeaveApplicationMassNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAuditCol("ELeaveApplicationMassDeti", UserNo, "LeaveApplicationMassNo", item)
                Generic.DeleteRecordAudit("ELeaveApplicationMass", UserNo, item)
                i = i + 1
            Next
    ...
    Protected Sub lnkDetail_Click(sender As Object, e As EventArgs)
        ...
        lnk = sender
        Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
        Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"LeaveApplicationMassNo", "LeaveApplicationMassTransNo"})
        ViewState("TransNo") = obj(0)
        lbl.Text = obj(1)
        PopulateGridDetl()
    End Sub
    ...
    Protected Sub lnkPost_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost, "DTRLeaveApplication.aspx", "ELeaveApplication") Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"LeaveApplicationMassNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                If SQLHelper.ExecuteNonQuery("ELeaveApplicationMass_WebForward", UserNo, item, PayLocNo) > 0 Then
                    Count = Count + 1
                End If
                i = i + 1
            Next
    ...
    Protected Sub lnkSave_Click(sender As Object, e As EventArgs)
        ...
        Dim LeaveApplicationMassNo As Integer = Generic.ToInt(txtLeaveApplicationMassNo.Text)
        ...
        dtx = SQLHelper.ExecuteDataTable("ELeaveApplication_WebValidate", UserNo, 0, 0, LeaveTypeNo, StartDate, EndDate, AppliedHrs, IsForAM, ApprovalStatNo, PayLocNo, ComponentNo)
        ...
        If SQLHelper.ExecuteNonQuery("ELeaveApplicationMass_WebSave", UserNo, LeaveApplicationMassNo, Description, LeaveTypeNo, StartDate, EndDate, AppliedHrs, IsForAM, PayLocNo, IsFromOnline) > 0 Then
            RetVal = True
        Else
            RetVal = False
        End If
...
#Region "********Details********"
    Protected Sub PopulateGridDetl()
        Try
            ...
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ELeaveApplicationMassDeti_Web", UserNo, Generic.ToInt(ViewState("TransNo")), "")
    ...
    Protected Sub lnkDeleteDetl_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete, "DTRLeaveApplication.aspx", "ELeaveApplication") Then
            Dim fieldValues As List(Of Object) = grdDetl.GetSelectedFieldValues(New String() {"LeaveApplicationMassDetiNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("ELeaveApplicationMassDeti", UserNo, item)
                i = i + 1
            Next
            MessageBox.Success("(" + i.ToString + ") " + MessageTemplate.SuccessDelete, Me)
            PopulateGridDetl()
...
#Region "********Append*******"
    ...
    Protected Sub lnkSaveAppend_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd, "DTRLeaveApplication.aspx", "ELeaveApplication") Then
            ...
            If SQLHelper.ExecuteNonQuery("ELeaveApplicationMassDeti_WebSave", UserNo, tno, FilteredbyNo, FilterValue) >= 0 Then
                Retval = True
            Else
                Retval = False
            End If
