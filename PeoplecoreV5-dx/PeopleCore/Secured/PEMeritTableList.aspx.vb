...
Partial Class Secured_PENormsList
    ...
    '            dt = SQLHelper.ExecuteDataTable("EPEMeritTable_Web", UserNo, Filter1.SearchText, PayLocNo)
    ...
    '            ib = sender
    '            Dim dt As DataTable
    '            dt = SQLHelper.ExecuteDataTable("EPEMeritTable_WebOne", UserNo, Generic.ToInt(ib.CommandArgument))
    ...
    '                    Generic.DeleteRecordAuditCol("EPEMeritTableDeti", UserNo, "PEMeritTableNo", Generic.ToInt(ib.CommandArgument))
    '                    Generic.DeleteRecordAudit("EPEMeritTable", UserNo, Generic.ToInt(ib.CommandArgument))
    '                    Count = Count + 1
    '                End If
    '            Next
    '            If Count > 0 Then
    ...
    '            Dim tno As Integer = Generic.ToInt(Me.txtPEMeritTableNo.Text)
    '            Dim PEPeriodNo As Integer = Generic.ToInt(Me.cboPEPeriodNo.SelectedValue)
    '            Dim ApplicableYear As Integer = Generic.ToStr(Me.txtApplicableyear.Text)
    '            Dim IsBonus As Boolean = Generic.ToBol(Me.txtIsBonus.Checked)
    '            Dim IsAmount As Boolean = Generic.ToBol(Me.txtIsAmount.Checked)
    '            Dim Remarks As String = Generic.ToStr(Me.txtRemark.Text)

    '            If SQLHelper.ExecuteNonQuery("EPEMeritTable_WebSave", UserNo, tno, PEPeriodNo, ApplicableYear, IsBonus, Remarks, IsAmount, PayLocNo) > 0 Then
    '                Retval = True
    '            Else
    '                Retval = False
    '            End If
    ...
    '            Dim dt As DataTable
    '            Dim sortDirection As String = "", sortExpression As String = ""
    '            dt = SQLHelper.ExecuteDataTable("EPEMeritTableDeti_Web", UserNo, Generic.ToInt(ViewState("TransNo")))
    '            Dim dv As DataView = dt.DefaultView
    ...
    '            ib = sender
    '            Dim dt As DataTable
    '            dt = SQLHelper.ExecuteDataTable("EPEMeritTableDeti_WebOne", UserNo, Generic.ToInt(ib.CommandArgument))
    '            For Each row As DataRow In dt.Rows
    ...
    '                If chk.Checked = True Then
    '                    Generic.DeleteRecordAudit("EPEMeritTableDeti", UserNo, Generic.ToInt(ib.CommandArgument))
    '                    Count = Count + 1
    '                End If
    '            Next
    '            If Count > 0 Then
    ...
    '        Dim PEMeritTableDetiNo As Integer = Generic.ToInt(Me.txtPEMeritTableDetiNo.Text)
    '        Dim PERatingNo As Integer = Generic.ToInt(Me.cboPERatingNo.SelectedValue)
    '        Dim PercentIncrease As Double = Generic.ToDec(Me.txtPercentIncrease.Text)
    '        Dim AmountIncrease As Double = Generic.ToDec(Me.txtAmountIncrease.Text)

    '        If SQLHelper.ExecuteNonQuery("EPEMeritTableDeti_WebSave", UserNo, PEMeritTableDetiNo, tno, PERatingNo, PercentIncrease, AmountIncrease) > 0 Then
    '            Retval = True
    '        Else
    '            Retval = False
    '        End If
    ...
    Private Sub PopulateGrid()
        ...
        _dt = SQLHelper.ExecuteDataTable("EPEMeritTable_Web", UserNo, PayLocNo)
    ...
    Private Sub PopulateGridDetl(id As Integer)
        Dim dt As DataTable
        dt = SQLHelper.ExecuteDataTable("EPEMeritTableDeti_Web", UserNo, id)
...
#Region "********Main*******"
    Protected Sub lnkEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit) Then
                ...
                'i = Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"PEMeritTableNo"}))
                Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
                Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"PEMeritTableNo", "IsEnabled"})
                Dim iNo As Integer = Generic.ToInt(obj(0))
                Dim IsEnabled As Boolean = Generic.ToBol(obj(1))

                Dim dt As DataTable
                dt = SQLHelper.ExecuteDataTable("EPEMeritTable_WebOne", UserNo, Generic.ToInt(iNo))
                For Each row As DataRow In dt.Rows
                    Generic.PopulateData(Me, "pnlPopupMain", dt)
                Next
                Try
    ...
    Protected Sub lnkDetail_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        lnk = sender
        Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
        Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"PEMeritTableNo", "PEMeritTableCode"})
        ViewState("TransNo") = obj(0)
        lblDetl.Text = "Reference No. : " & obj(1)
        PopulateGridDetl(Generic.ToInt(ViewState("TransNo")))
    End Sub
    ...
    Protected Sub lnkDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkDelete.Click
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"PEMeritTableNo"})
            Dim str As String = ""
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EPEMeritTable", UserNo, item)
                Generic.DeleteRecordAuditCol("EPEMeritTableDeti", UserNo, "PEMeritTableNo", item)
    ...
    Private Function SaveRecord() As Integer
        ...
        If SQLHelper.ExecuteNonQuery("EPEMeritTable_WebSave", UserNo, tno, PEPeriodNo, ApplicableYear, IsBonus, Remarks, IsAmount, Generic.ToInt(cboPayLocNo.SelectedValue)) > 0 Then
            Retval = True
        Else
            Retval = False
        End If
...
#Region "********Detail********"
    ...
    Protected Sub lnkDeleteDetl_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        TransNo = Generic.ToInt(ViewState("TransNo"))
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            Dim fieldValues As List(Of Object) = grdDetl.GetSelectedFieldValues(New String() {"PEMeritTableDetiNo"})
            Dim str As String = ""
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EPEMeritTableDeti", UserNo, item)
                DeleteCount = DeleteCount + 1
            Next
