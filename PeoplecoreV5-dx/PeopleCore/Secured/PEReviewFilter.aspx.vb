Imports clsLib
Imports System.Data
Partial Class Secured_PEReviewFilter
...
#Region "Position"
    Protected Sub PopulateGrid()
        Try
            ...
            dt = SQLHelper.ExecuteDataTable("EPEReviewFilter_Web", UserNo, TransNo, "Position")
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("EPEReviewFilter_WebOne", UserNo, id)
            For Each row As DataRow In dt.Rows
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete) Then
            For i As Integer = 0 To Me.grdMain.Rows.Count - 1
                ...
                If chk.Checked = True Then
                    Generic.DeleteRecordAudit("EPEReviewFilter", UserNo, Generic.ToInt(ib.CommandArgument))
                    Count = Count + 1
                End If
            Next
    ...
    Protected Sub lnkSave_Click(sender As Object, e As EventArgs)
        ...
        Dim tno As Integer = Generic.ToInt(Me.txtPEReviewFilterNo.Text)
        ...
        If SQLHelper.ExecuteNonQuery("EPEReviewFilter_WebSave", UserNo, tno, TransNo, "Position", JobGradeNo, Position, EmployeeStatNo) > 0 Then
            Retval = True
        Else
            Retval = False
        End If
...
#Region "Job Level"
    Protected Sub PopulateLevel()
        Try
            Dim dt As DataTable
            Dim sortDirection As String = "", sortExpression As String = ""
            dt = SQLHelper.ExecuteDataTable("EPEReviewFilter_Web", UserNo, TransNo, "Level")
            Dim dv As DataView = dt.DefaultView
    ...
    Protected Sub lnkSaveLevel_Click(sender As Object, e As EventArgs)
        ...
        If SQLHelper.ExecuteNonQuery("EPEReviewFilter_WebSave", UserNo, tno, TransNo, "Level", JobGradeNo, Position, EmployeeStatNo) > 0 Then
            Retval = True
        Else
            Retval = False
        End If
...
#Region "Employee Status"
    Protected Sub PopulateStatus()
        Try
            Dim dt As DataTable
            Dim sortDirection As String = "", sortExpression As String = ""
            dt = SQLHelper.ExecuteDataTable("EPEReviewFilter_Web", UserNo, TransNo, "Status")
            Dim dv As DataView = dt.DefaultView
    ...
    Protected Sub lnkSaveStatus_Click(sender As Object, e As EventArgs)
        Dim Retval As Boolean = False
        Dim tno As Integer = Generic.ToInt(Me.txtPEReviewFilterStatusNo.Text)
        Dim JobGradeNo As Integer = 0
        Dim Position As Integer = 0
        Dim EmployeeStatNo As Integer = Generic.ToInt(Me.cboEmployeeStatNo.SelectedValue)

        If SQLHelper.ExecuteNonQuery("EPEReviewFilter_WebSave", UserNo, tno, TransNo, "Status", JobGradeNo, Position, EmployeeStatNo) > 0 Then
            Retval = True
        Else
            Retval = False
        End If
