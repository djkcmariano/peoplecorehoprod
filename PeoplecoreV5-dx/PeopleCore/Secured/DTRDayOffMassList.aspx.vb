...
Partial Class Secured_DTRDayOffMassList
...
#Region "********Main********"
    Protected Sub PopulateGrid(Optional IsMain As Boolean = False)
        Try
            ...
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("EDTRDayOffMass_Web", UserNo, Generic.ToInt(cboTabNo.SelectedValue), PayLocNo)
            grdMain.DataSource = dt
            grdMain.DataBind()

            If ViewState("TransNo") = 0 Or IsMain = True Then
                Dim obj As Object() = grdMain.GetRowValues(grdMain.VisibleStartIndex(), New String() {"DTRDayOffMassNo", "DTRDayOffMassTransNo"})
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("EDTRDayOffMass_WebOne", UserNo, id)
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        AccessRights.CheckUser(UserNo, "DTRDayOff.aspx", "EDTRDayOff")
    ...
    Protected Sub lnkAdd_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd, "DTRDayOff.aspx", "EDTRDayOff") Then
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit, "DTRDayOff.aspx", "EDTRDayOff") Then
            Dim lnk As New LinkButton
            lnk = sender
            Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
            PopulateData(Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"DTRDayOffMassNo"})))
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete, "DTRDayOff.aspx", "EDTRDayOff") Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"DTRDayOffMassNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EDTRDayOffMass", UserNo, item)
                i = i + 1
            Next
    ...
    Protected Sub lnkDetail_Click(sender As Object, e As EventArgs)
        ...
        lnk = sender
        Dim container As GridViewDataItemTemplateContainer = TryCast(lnk.NamingContainer, GridViewDataItemTemplateContainer)
        Dim obj As Object() = container.Grid.GetRowValues(container.VisibleIndex, New String() {"DTRDayOffMassNo", "DTRDayOffMassTransNo"})
        ViewState("TransNo") = obj(0)
        lbl.Text = obj(1)
        PopulateGridDetl()
    End Sub
    ...
    Protected Sub lnkPost_Click(sender As Object, e As EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowPost, "DTRDayoff.aspx", "EDTRDayoff") Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"DTRDayOffMassNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                If SQLHelper.ExecuteNonQuery("EDTRDayOffMass_WebForward", UserNo, item, PayLocNo) > 0 Then
                    Count = Count + 1
                End If
                i = i + 1
            Next
    ...
    Protected Sub lnkSave_Click(sender As Object, e As EventArgs)
        ...
        Dim DTRDayOffMassNo As Integer = Generic.ToInt(txtDTRDayOffMassNo.Text)
        ...
        dtx = SQLHelper.ExecuteDataTable("EDTRDayOff_WebValidate", UserNo, 0, 0, DateFrom, DateTo, DayOffNo, DayOffNo2, DayOffNo3, ApprovalStatNo, PayLocNo, ComponentNo)
        ...
        If SQLHelper.ExecuteNonQuery("EDTRDayOffMass_WebSave", UserNo, DTRDayOffMassNo, Description, DateFrom, DateTo, DayOffNo, DayOffNo2, DayOffNo3, PayLocNo, IsFromOnline) > 0 Then
            RetVal = True
        Else
            RetVal = False
        End If
...
#Region "********Details********"
    Protected Sub PopulateGridDetl()
        Try
            ...
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("EDTRDayoffMassDeti_Web", UserNo, Generic.ToInt(ViewState("TransNo")), "")
    ...
    Protected Sub lnkDeleteDetl_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete, "DTRDayoff.aspx", "EDTRDayoff") Then
            Dim fieldValues As List(Of Object) = grdDetl.GetSelectedFieldValues(New String() {"DTRDayOffMassDetiNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("EDTRDayOffMassDeti", UserNo, item)
                i = i + 1
            Next
            MessageBox.Success("(" + i.ToString + ") " + MessageTemplate.SuccessDelete, Me)
            PopulateGridDetl()
...
#Region "********Append*******"
    ...
    Protected Sub lnkSaveAppend_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowAdd, "DTRDayOff.aspx", "EDTRDayOff") Then
            ...
            If SQLHelper.ExecuteNonQuery("EDTRDayOffMassDeti_WebSave", UserNo, tno, FilteredbyNo, FilterValue) >= 0 Then
                Retval = True
            Else
                Retval = False
            End If
