...
Partial Class Secured_TrnMatrixList
    ...
    Protected Sub PopulateGrid(Optional IsMain As Boolean = False)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ETrnCurriculum_Web", UserNo, Generic.ToInt(cboTabNo.SelectedValue), "", PayLocNo)
            grdMain.DataSource = dt
            grdMain.DataBind()

            If ViewState("TransNo") = 0 Or IsMain = True Then
                Dim obj As Object() = grdMain.GetRowValues(grdMain.VisibleStartIndex(), New String() {"EmployeeNo", "FullName"})
    ...
    Private Sub PopulateDetl()
        ...
        imgPhoto.ImageUrl = "~/secured/frmShowImage.ashx?tNo=" & Generic.ToInt(ViewState("TransNo")) & "&tIndex=2"

        Dim dt As DataTable
        dt = SQLHelper.ExecuteDataTable("ETrnMatrix_Web", UserNo, Generic.ToInt(ViewState("TransNo")), 0, 1, PayLocNo)
    ...
    Private Sub PopulateTab(Optional Index As Integer = 0)
        ...
        ElseIf Index = 5 Then 'No Show
            ...
            grdDetl.Columns("Status").Visible = False
            grdDetl.Columns("Reason").Visible = True
        ElseIf Index = 6 Then 'Training Bond
            lnkTaken.CssClass = "list-group-item text-left"
            lnkRequired.CssClass = "list-group-item text-left"
            lnkCompleted.CssClass = "list-group-item text-left"
            lnkIncomplete.CssClass = "list-group-item text-left"
