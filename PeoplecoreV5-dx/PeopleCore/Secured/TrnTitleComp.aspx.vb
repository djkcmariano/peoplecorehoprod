...
Partial Class Secured_TrnTitleEditComp
    ...
    Protected Sub PopulateGrid()
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ETrnTitleComp_Web", UserNo, TransNo)
    ...
    Protected Sub PopulateData(id As Int64)
        Try
            Dim dt As DataTable
            dt = SQLHelper.ExecuteDataTable("ETrnTitleComp_WebOne", UserNo, id)
            For Each row As DataRow In dt.Rows
    ...
    Protected Sub lnkSave_Click(sender As Object, e As EventArgs)
        ...
        dtx = SQLHelper.ExecuteDataTable("ETrnTitleComp_WebValidate", UserNo, Generic.ToInt(txtCode.Text), TransNo, Generic.ToInt(cboCompNo.SelectedValue), Generic.ToInt(Me.cboCompScaleNo.SelectedValue), Me.txtAnchor.Text)
        ...
        If SQLHelper.ExecuteNonQuery("ETrnTitleComp_WebSave", UserNo, Generic.ToInt(txtCode.Text), TransNo, Generic.ToInt(cboCompNo.SelectedValue), Generic.ToInt(Me.cboCompScaleNo.SelectedValue), Me.txtAnchor.Text, chkIsArchived.Checked, txtEffectiveDate.Text) > 0 Then
            Retval = True
        Else
            Retval = False
        End If
    ...
    Protected Sub lnkEdit_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowEdit, "TrnTitleList.aspx", "ETrnTitle") Then
            ...
            PopulateData(Generic.ToInt(container.Grid.GetRowValues(container.VisibleIndex, New String() {"TrnTitleCompNo"})))
            ModalPopupExtender1.Show()
        Else
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        If AccessRights.IsAllowUser(UserNo, AccessRights.EnumPermissionType.AllowDelete, "TrnTitleList.aspx", "ETrnTitle") Then
            Dim fieldValues As List(Of Object) = grdMain.GetSelectedFieldValues(New String() {"TrnTitleCompNo"})
            Dim str As String = "", i As Integer = 0
            For Each item As Integer In fieldValues
                Generic.DeleteRecordAudit("ETrnTitleComp", UserNo, item)
                i = i + 1
            Next
